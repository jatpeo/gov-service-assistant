...
public class StandardAnswerService {
    ...
    private static final List<StandardAnswerRule> RULES = List.of(
            new StandardAnswerRule(
                    "GREETING",
                    Conversation.IntentType.OTHER,
                    1.0,
                    Arrays.asList(
                            Pattern.compile("^(你好|您好|嗨|hello|hi|早上好|上午好|中午好|下午好|晚上好)[!！。,.，\\s]*$")
                    ),
                    """
                            您好，我是智能客服助手，很高兴为您服务。

                            您可以直接告诉我想咨询的事项，例如政策咨询、业务办理、进度查询、账号权限或系统使用问题。我会尽量帮您快速找到答案；如果需要，也可以为您转接人工客服。
                            """
            ),
            new StandardAnswerRule(
                    "LOGIN_PASSWORD",
